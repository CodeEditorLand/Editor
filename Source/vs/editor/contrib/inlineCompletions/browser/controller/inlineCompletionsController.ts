			...
			runOnChange(this._editorObs.selections, (_value, _, changes) => {
				if (
					...
					if (
						...
						this.model.get()?.stateWithInlineEdit.get()?.kind ===
							"inlineEdit"
					) {
						return;
					}
	...
	font-family: ${fontFamily};
		...
		this._register(
			bindContextKey(
				...
				(reader) => {
					const cursorPos =
						this._editorObs.cursorPosition.read(reader);
					if (cursorPos === null) {
						return false;
					}
					const edit = this.model
						.read(reader)
						?.stateInlineEdit.read(reader);
					if (!edit) {
						return false;
					}
					return LineRange.fromRangeInclusive(
						edit.inlineEdit.range,
