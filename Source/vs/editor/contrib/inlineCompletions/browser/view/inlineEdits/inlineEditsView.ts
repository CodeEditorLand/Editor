...
export class InlineEditsViewAndDiffProducer extends Disposable {
	...
	private readonly _inlineEditPromise = derivedWithCancellationToken<
		ObservablePromise<InlineEditWithChanges | undefined> | undefined
	>(this, (reader, token) => {
		...
		this._originalModel
			.get()
			.setValue(this._editor.getModel()!.getValueInRange(edit.range));
	...
	private readonly _inlineEdit = this._inlineEditPromise.map(
		(p, reader) => p?.promiseResult?.read(reader)?.data,
	);
...
export class InlineEditsView extends Disposable {
	...
	constructor(
		...
		this._register(
			applyStyle(this._elements.root, {
				display: derived(this, (reader) =>
					!!this._diffViewInformation.read(reader) ? "block" : "none",
				),
			}),
		);
		...
		this._register(
			observableCodeEditor(_editor).setDecorations(
		...
		this._register(
			autorun((reader) => {
				...
				this._elements.path.style.stroke =
	...
	private readonly _uiState = derived(this, (reader) => {
		...
		const originalDisplayRange = LineRange.ofLength(
