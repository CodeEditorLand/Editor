...
class ManageAccountPreferenceForExtensionActionImpl {
	...
	async run(extensionId?: string, providerId?: string) {
			...
			for (const providerId of this._authenticationService.getProviderIds()) {
				...
				for (const account of accounts) {
					const usage = this._authenticationUsageService
						...
						.find(
							(u) => u.extensionId === extensionId.toLowerCase(),
						);
					if (usage) {
						providerIds.push(providerId);
		...
		if (providerIds.length > 1) {
			const result = await this._quickInputService.pick(
				providerIds.map((providerId) => ({
					label: this._authenticationService.getProvider(providerId)
						.label,
					id: providerId,
				})),
				{
		...
		const provider =
			this._authenticationService.getProvider(chosenProviderId);
	...
	private _getItems(
		...
		return accounts.map<QuickPickInput<AccountPreferenceQuickPickItem>>(
			(a) =>
