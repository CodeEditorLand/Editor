	...
	constructor() {
		super({
			...
			precondition: ContextKeyExpr.and(
				...
				ContextKeyExpr.or(
					CONTEXT_CHAT_LOCATION.notEqualsTo(
...
MenuRegistry.appendMenuItem(MenuId.ChatExecute, {
	...
	when: ContextKeyExpr.and(
		...
		CONTEXT_PARTICIPANT_SUPPORTS_MODEL_PICKER,
		ContextKeyExpr.or(
			ContextKeyExpr.equals(CONTEXT_CHAT_LOCATION.key, "panel"),
			ContextKeyExpr.equals(
				CONTEXT_CHAT_LOCATION.key,
				ChatAgentLocation.EditingSession,
			),
		),
	),
...
export class CancelAction extends Action2 {
	...
	run(accessor: ServicesAccessor, ...args: any[]) {
		...
		if (
			...
			currentEditingSession?.chatSessionId === widget.viewModel?.sessionId
		) {
