...
export class ApplyCodeBlockOperation {
	...
	public async run(context: ICodeBlockActionContext): Promise<void> {
		...
		if (
			...
			!isEqual(activeEditorControl?.getModel().uri, context.codemapperUri)
		) {
	...
	): Promise<IComputeEditsResult> {
		const activeModel = codeEditor.getModel();

		const mappedEditsProviders =
			this.languageFeaturesService.mappedEditsProvider.ordered(
				activeModel,
			);
...
function collectDocumentContextFromSelections(
	codeEditor: IActiveCodeEditor,
...
function collectDocumentContextFromContext(
	context: ICodeBlockActionContext,
	result: DocumentContextItem[][],
): void {
	if (
