					...
					if (!rendererOptions.renderCodeBlockPills) {
						...
						const info: IChatCodeBlockInfo = new (class {
							...
							readonly isStreaming =
								!rendererOptions.renderCodeBlockPills;
	...
	private renderCodeBlock(
		...
		if (isResponseVM(data.element)) {
			this.codeBlockModelCollection
				...
				.then((e) => {
					// Update the existing object's codemapperUri
					this.codeblocks[data.codeBlockIndex].codemapperUri =
						e.codemapperUri;
					this._onDidChangeHeight.fire();
				});
		}
		...
		this.allRefs.forEach((ref, index) => {
			if (ref.object instanceof CodeBlockPart) {
				...
				if (
					...
					ref.object.uri?.toString() !==
						codeblockModel.codemapperUri.toString()
				) {
					ref.object.render(
						codeblockModel.codemapperUri,
	...
	constructor(
		...
		this._register(
			dom.addDisposableListener(this.element, "click", async () => {
				if (this.uri) {
					const snapshot = this.chatEditingService.getSnapshotUri(
						this.requestId,
						this.uri,
					);
					if (snapshot) {
