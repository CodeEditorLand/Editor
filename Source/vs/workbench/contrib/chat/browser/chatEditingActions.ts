...
registerAction2(
	class RemoveFileFromWorkingSet extends WorkingSetAction {
		...
		async runWorkingSetAction(
			...
			for (const context of chatWidget.input.attachmentModel
				.attachments) {
				if (
					!URI.isUri(context.value) ||
					!context.isFile ||
					!resourceSet.has(context.value)
				) {
...
registerAction2(
	class OpenFileAction extends WorkingSetAction {
		constructor() {
			super({
...
registerAction2(
	class AddFilesToWorkingSetAction extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.addFilesToWorkingSet",
				title: localize2(
					"workbench.action.chat.addFilesToWorkingSet.label",
...
registerAction2(
	class RestoreWorkingSetAction extends Action2 {
		constructor() {
			super({
				id: "workbench.action.chat.restoreWorkingSet",
				title: localize2(
					"chat.restoreWorkingSet.label",
					"Restore Working Set",
				),
				f1: false,
				shortTitle: localize2(
					"chat.restoreWorkingSet.shortTitle",
					"Restore Working Set",
				),
				toggled: {
					condition: isChatRequestCheckpointed,
					title: localize2(
						"chat.restoreWorkingSet.title",
						"Using Working Set",
					).value,
					tooltip: localize2(
						"chat.restoreWorkingSet.tooltip",
						"Toggle to use the working set state from an earlier request in your next edit",
					).value,
				},
				precondition: ContextKeyExpr.and(
					applyingChatEditsContextKey.negate(),
