	...
	implements IWorkbenchContribution
{
	...
	private readonly _sessionStore = this._store.add(
		new DisposableMap<IChatEditingSession>(),
	);
		...
		const update = () => {
			...
			if (chatEditingService.currentEditingSession) {
				this._handleNewEditingSession(
					chatEditingService.currentEditingSession,
					store,
				);
			}

			store.add(
				chatEditingService.onDidCreateEditingSession((e) =>
				...
				textFileService.files.addSaveParticipant({
					participate: async (
						...
						await queue.queue(async () => {
							...
							const filelabel = labelService.getUriBasenameLabel(
								workingCopy.resource,
							);
							...
							const message = agentName
								...
								: localize(
										"message.2",
										"Do you want to save the changes chat made in {0}?",
										filelabel,
									);
							...
							const result = await this._dialogService.confirm({
								...
								detail: localize(
									"detail",
									"AI-generated changes may be incorect and should be reviewed before saving.",
									agentName,
								),
		...
		const update = () => {
			...
			for (const entry of entries) {
				if (entry.state.get() === WorkingSetEntryState.Modified) {
					store.add(
						this._fileConfigService.disableAutoSave(
							entry.modifiedURI,
						),
					);
				}
