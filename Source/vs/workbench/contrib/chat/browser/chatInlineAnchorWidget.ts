...
const chatResourceContextKey = new RawContextKey<string>(
	"chatAnchorResource",
	undefined,
	{
		type: "URI",
		description: localize(
			"resource",
			"The full value of the chat anchor resource, including scheme and path",
		),
	},
);
...
export class InlineAnchorWidget extends Disposable {
	...
	constructor(
		...
		this._chatResourceContext =
			chatResourceContextKey.bindTo(contextKeyService);
		...
		let contextMenuArg:
			| URI
			| { readonly uri: URI; readonly range?: IRange };
		if (data.kind === "symbol") {
			...
			if (model) {
				const hasDefinitionProvider =
					EditorContextKeys.hasDefinitionProvider.bindTo(
						contextKeyService,
					);
				const hasReferenceProvider =
			...
			this._register(
				dom.addDisposableListener(element, "click", () => {
			...
			iconText =
				location.range && data.kind !== "symbol"
					? `${label}#${location.range.startLineNumber}-${location.range.endLineNumber}`
					: label;

			const fileKind = location.uri.path.endsWith("/")
				? FileKind.FOLDER
				: FileKind.FILE;
			...
			const isFolderContext =
				ExplorerFolderContext.bindTo(contextKeyService);
			fileService
				.stat(location.uri)
				.then((stat) => {
		...
		this._register(
			dom.addDisposableListener(element, "dragstart", (e) => {
				instantiationService.invokeFunction((accessor) =>
					fillEditorsDragData(accessor, [location.uri], e),
				);
