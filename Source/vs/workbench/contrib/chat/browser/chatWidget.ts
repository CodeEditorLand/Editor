...
export class ChatWidget extends Disposable implements IChatWidget {
	...
	constructor(
		...
		this.agentSupportsModelPicker =
			CONTEXT_PARTICIPANT_SUPPORTS_MODEL_PICKER.bindTo(contextKeyService);
		...
		const chatEditingSessionDisposables = this._register(
			new DisposableStore(),
		);

		this._register(
			this.chatEditingService.onDidCreateEditingSession((session) => {
				...
				chatEditingSessionDisposables.add(
					session.onDidChange(() => {
		...
		if (this._location.location === ChatAgentLocation.EditingSession) {
			...
			this._register(
				this.onDidChangeViewModel(async () => {
					const sessionId = this._viewModel?.sessionId;
					if (sessionId !== currentEditSession?.chatSessionId) {
						if (
							currentEditSession &&
							currentEditSession.state.get() !==
								ChatEditingSessionState.Disposed
						) {
							await currentEditSession.stop();
						}
						if (sessionId) {
							currentEditSession =
								await this.chatEditingService.startOrContinueEditingSession(
									sessionId,
									{ silent: true },
								);
						} else {
	...
	render(parent: HTMLElement): void {
		...
		this.welcomeMessageContainer = dom.append(
			this.container,
			$(".chat-welcome-view", { style: "display: none" }),
		);
	...
	private onDidChangeItems(skipDynamicLayout?: boolean) {
		if (this.tree && this._visible) {
			...
			if (lastItem) {
				CONTEXT_LAST_ITEM_ID.bindTo(this.contextKeyService).set([
					lastItem.id,
				]);
			}
	...
	private renderWelcomeViewContentIfNeeded() {
		const welcomeContent =
			this.viewModel?.model.welcomeMessage ??
			this.persistedWelcomeMessage;
		if (
			welcomeContent &&
			this.welcomeMessageContainer.children.length === 0 &&
			!this.viewOptions.renderStyle
		) {
			...
			const messageResult = this._register(
				renderer.render(welcomeContent.message),
			);
	...
	private async renderChatEditingSessionState(
		session: IChatEditingSession | null,
	) {
	...
	private createInput(
		...
		this._register(
			this.inputPart.attachmentModel.onDidChangeContext(() => {
				if (
					this.chatEditingService.currentEditingSession &&
					this.chatEditingService.currentEditingSession
						?.chatSessionId === this.viewModel?.sessionId
				) {
					...
					if (
						...
						for (let i = 0; i < requests.length - 1; i++) {
							if (requests[i + 1].id === e.request.id) {
								...
								this.chatEditingService.createSnapshot(
									requests[i].id,
								);
								break;
							}
	...
	layout(height: number, width: number): void {
		...
		const followupsOffset = Math.max(
			100 - this.inputPart.followupsHeight,
			0,
		);
	...
	saveState(): void {
		...
		if (this.viewModel?.model.welcomeMessage) {
			this.storageService.store(
				PersistWelcomeMessageContentKey,
				this.viewModel?.model.welcomeMessage,
				StorageScope.APPLICATION,
	...
	private updateChatInputContext() {
		...
		this.agentSupportsModelPicker.set(
			!currentAgent || !!currentAgent.agent.supportsModelPicker,
		);
	}
}
...
export class ChatWidgetService implements IChatWidgetService {
	...
	get lastFocusedWidget(): IChatWidget | undefined {
		return (
			TerminalChatController.activeChatController?.chatWidget ??
