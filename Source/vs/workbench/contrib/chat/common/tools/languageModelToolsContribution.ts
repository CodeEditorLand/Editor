			...
			items: {
				...
				properties: {
					...
					tags: {
						description: localize(
							"toolTags",
							"A set of tags that roughly describe the tool's capabilities. A tool user may use these to filter the set of tools to just ones that are relevant for the task at hand.",
						),
						type: "array",
						items: {
							type: "string",
				...
				for (const rawTool of extension.value) {
					...
					const tool: IToolData = {
						...
						when: rawTool.when
							...
							: undefined,
						supportedContentTypes: rawTool.supportedContentTypes
