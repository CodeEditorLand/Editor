...
export class CommentController implements IEditorContribution {
	...
	private async resumePendingComment(
		...
		if (thread.isReply && matchedZones.length) {
			...
			if (
				...
				existingPendingComment.body.includes(thread.comment.body)
			) {
	...
	private getWithoutCommentsEditorOptions(
		...
		if (
			...
			lineDecorationsWidth += 11; // 11 comes from https://github.com/microsoft/vscode/blob/94ee5f58619d59170983f453fe78f156c0cc73a3/Source/vs/workbench/contrib/comments/browser/media/review.css#L485
		}
	...
	private async setComments(commentInfos: ICommentInfo[]): Promise<void> {
		...
		for (const info of this._commentInfos) {
			...
			for (const thread of info.threads) {
				let pendingComment: languages.PendingComment | undefined =
					undefined;
	...
	private removeCommentWidgetsAndStoreCache() {
		if (this._commentWidgets) {
			this._commentWidgets.forEach((zone) => {
				...
				if (
					pendingNewComment &&
					pendingNewComment.body !== lastCommentBody
				) {
