...
function collapseDeepestExpandedLevel(accessor: ServicesAccessor) {
	...
	if (searchView) {
		...
		if (
			node instanceof FolderMatchWorkspaceRoot ||
				...
				if (
					...
					if (node instanceof FolderMatch) {
						...
						nodeToTest =
							...
							!(compressionStartNode instanceof Match) &&
							!(
								compressionStartNode instanceof TextSearchResult
							) &&
							!(compressionStartNode instanceof SearchResult)
								? compressionStartNode
								: node;
					}
					...
					if (
						!(
							immediateParent instanceof TextSearchResult ||
							immediateParent instanceof
								FolderMatchWorkspaceRoot ||
							immediateParent instanceof FolderMatchNoRoot ||
							immediateParent instanceof SearchResult
						)
					) {
		...
		if (
			firstFocusParent &&
			(firstFocusParent instanceof FolderMatch ||
				firstFocusParent instanceof FileMatch) &&
