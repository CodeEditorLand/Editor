...
export class SearchView extends ViewPane {
	...
	private async refreshTree(event?: IChangeEvent): Promise<void> {
		this.refreshTreePromiseSerializer =
			this.refreshTreePromiseSerializer.then(async () => {
				if (!event || event.added || event.removed) {
					...
					if (
						...
						await this.retrieveFileStats().then(() =>
							this.tree.updateChildren(undefined),
						);
					} else {
			...
			this.instantiationService.createInstance(
				...
				delegate,
				{
					isIncompressible: (element: RenderableMatch) => {
						if (
							element instanceof FolderMatch &&
							!(element.parent() instanceof TextSearchResult) &&
							!(
								element.parent() instanceof
								FolderMatchWorkspaceRoot
							) &&
							!(element.parent() instanceof FolderMatchNoRoot)
						) {
							return false;
						}
						return true;
	...
	private hasSomeCollapsible(): boolean {
		...
		do {
			if (
				...
				(!shouldShowAI || !(node instanceof TextSearchResult))
			) {
...
class SearchViewDataSource
	implements IAsyncDataSource<SearchResult, RenderableMatch>
{
	constructor(
	...
	): Iterable<FolderMatch | FileMatch> {
	...
	hasChildren(element: RenderableMatch): boolean {
		...
		if (
			element instanceof TextSearchResult &&
			element.id() === AI_TEXT_SEARCH_RESULT_ID
		) {
			return true;
		}
	...
	getChildren(
		element: RenderableMatch | SearchResult,
