	...
	implements ITerminalContribution
{
	...
	async acceptInput(
		...
		let responseContent = "";
		const response =
			await this._terminalChatWidget.value.inlineChatWidget.chatWidget.acceptInput(
				lastInput,
				isVoiceInput,
			);
					...
					response.onDidChange(async () => {
						...
						if (response.isComplete) {
							...
							const firstCodeBlock =
								await this.terminalChatWidget?.inlineChatWidget.getCodeBlockInfo(
									0,
								);
							...
							this._terminalChatWidget?.value.inlineChatWidget.updateToolbar(
								true,
							);
	...
	populateHistory(up: boolean) {
		...
		if (this._historyOffset === -1) {
			...
			this._historyCandidate =
				this._terminalChatWidget.value.inlineChatWidget.value;
		}
	...
	cancel(): void {
		...
		this._requestActiveContextKey.set(false);
		const model =
			this._terminalChatWidget?.value.inlineChatWidget.getChatModel();
		if (!model?.sessionId) {
			return;
		}
	...
	async acceptCommand(shouldExecute: boolean): Promise<void> {
		const code =
			await this.terminalChatWidget?.inlineChatWidget.getCodeBlockInfo(0);
		if (!code) {
			return;
		}
		this._terminalChatWidget?.value.acceptCommand(
