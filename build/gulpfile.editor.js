	...
	standalone.extractEditor({
		...
		redirects: {
			"@vscode/tree-sitter-wasm":
				"../node_modules/@vscode/tree-sitter-wasm/wasm/tree-sitter-web",
		},
	});
});
...
const compileEditorAMDTask = task.define(
	"compile-editor-amd",
	compilation.compileTask("out-editor-src", "out-editor-build", true, {
		disableMangle: true,
		preserveEnglish: true,
	}),
);

const bundleEditorAMDTask = task.define(
	"bundle-editor-amd",
	optimize.bundleTask({
		out: "out-editor",
		esm: {
...
const minifyEditorAMDTask = task.define(
	"minify-editor-amd",
	optimize.minifyTask("out-editor"),
);

const createESMSourcesAndResourcesTask = task.define(
	"extract-editor-esm",
	() => {
...
const compileEditorESMTask = task.define("compile-editor-esm", () => {
	...
	console.log(
		`Launching the TS compiler at ${path.join(__dirname, "../out-editor-esm")}...`,
	);
	let result;
	...
	if (FAIL_ON_PURPOSE || result.status !== 0) {
		...
		const cleanDestPath = keepPrevAnalysis
			? Promise.resolve()
			: util.rimraf(destPath)();
		return cleanDestPath.then(() => {
			// build a list of files to copy
			const files = util.rreddir(
				path.join(__dirname, "../out-editor-esm"),
			);
			...
			console.log(
				`Open in VS Code the folder at '${destPath}' and you can analyze the compilation error`,
			);
			throw new Error(
				"Standalone Editor compilation failed. If this is the build machine, simply launch `npm run gulp editor-distro` on your machine to further analyze the compilation problem.",
			);
		});
	}
});
...
const finalEditorResourcesTask = task.define("final-editor-resources", () => {
	return es.merge(
					...
					this.emit(
						...
						new File({
							...
							contents: Buffer.from(
								toExternalDTS(data.contents.toString()),
							),
						}),
					);
				}),
		...
		// dev folder
		es
			.merge(gulp.src("out-editor/**/*"))
			.pipe(gulp.dest("out-monaco-editor-core/dev")),

		// min folder
		es
			.merge(gulp.src("out-editor-min/**/*"))
			.pipe(
				filterStream(function (path) {
					// no map files
					return !/(\.js\.map$)|(nls\.metadata\.json$)|(bundleInfo\.json$)/.test(
						path,
					);
				}),
			)
...
gulp.task(
	...
	task.series(
		...
		task.parallel(
			...
			task.series(createESMSourcesAndResourcesTask, compileEditorESMTask),
		),
