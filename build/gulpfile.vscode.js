	...
	return () => {
		...
		if (platform === "win32") {
			...
			result = es.merge(
				result,
				gulp
					.src("resources/win32/bin/code.cmd", {
						base: "resources/win32",
					})
			...
			result = es.merge(
				result,
				gulp
					.src("resources/win32/bin/code.sh", {
						base: "resources/win32",
					})
					.pipe(replace("@@NAME@@", product.nameShort))
...
gulp.task(
	task.define(
		...
		task.series(core, compileExtensionsBuildTask, function () {
