...
function packageTask(sourceFolderName, destinationFolderName) {
	...
	return () => {
		...
		const loader = gulp
			.src("build/loader.min", { base: "build", dot: true })
			.pipe(rename("out/vs/loader.js")); // TODO@esm remove line when we stop supporting web-amd-esm-bridge

		const sources = es
			.merge(src, extensions, loader)
		...
		const pwaicons = es.merge(
			gulp.src("resources/server/code-192.png", {
				base: "resources/server",
			}),
