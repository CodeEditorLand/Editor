...
function fromLocalWebpack(extensionPath, webpackConfigFileName, disableMangle) {
    ...
    vsce.listFiles({
        ...
        const webpackStreams = webpackConfigLocations.flatMap((webpackConfigPath) => {
            ...
            return (Array.isArray(exportedConfig)
                ? exportedConfig
                : [exportedConfig]).map((config) => {
                const webpackConfig = {
                    ...config,
                    ...{ mode: "production" },
                };
...
/**
 * Loosely based on `getExtensionKind` from `Source/vs/workbench/services/extensions/common/extensionManifestPropertiesService.ts`
 */
...
function packageLocalExtensionsStream(forWeb, disableMangle) {
    ...
    if (forWeb) {
        ...
        const dependenciesSrc = productionDependencies
            ...
            .flat();
        result = es.merge(localExtensionsStream, gulp
...
function packageMarketplaceExtensionsStream(forWeb) {
    ...
    const marketplaceExtensionsStream = minifyExtensionResources(es.merge(...marketplaceExtensionsDescriptions.map((extension) => {
    ...
    try {
        ...
        for (const extensionFolder of extensionsFolders) {
            ...
            const packageNLS = packageNLSPath
                ? JSON.parse(fs
                    .readFileSync(path.join(extensionsRoot, extensionFolder, packageNLSPath))
                    .toString())
                : undefined;
            ...
            scannedExtensions.push({
                ...
                packageNLS,
                readmePath: readme
                    ? path.join(extensionFolder, readme)
                    : undefined,
