			...
			const webpackStreams = webpackConfigLocations.flatMap(
				(webpackConfigPath) => {
					const webpackDone = (err: any, stats: any) => {
						fancyLog(
							`Bundled extension: ${ansiColors.yellow(path.join(path.basename(extensionPath), path.relative(extensionPath, webpackConfigPath)))}...`,
						);
						if (err) {
		...
		if (Array.isArray(fullStats.children)) {
			for (const stats of fullStats.children) {
				...
				if (outputPath) {
					...
					fancyLog(
						`Finished ${ansiColors.green(taskName)} ${ansiColors.cyan(match![0])} with ${stats.errors.length} errors.`,
					);
				}
