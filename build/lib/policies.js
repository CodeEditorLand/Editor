...
const PolicyTypes = [BooleanPolicy, IntPolicy, StringEnumPolicy, StringPolicy];
...
function getPolicies(moduleName, node) {
    ...
    return query.matches(node).map((m) => {
        ...
        const settingNode = m.captures.filter((c) => c.name === "setting")[0]
            .node;
        const policyNode = m.captures.filter((c) => c.name === "policy")[0]
            .node;
    ...
    return {
        ...
        adml: [
            ...
            ...translations.map(({ languageId, languageTranslations }) => ({
                languageId,
                contents: renderADML(appName, versions, categories, policies, languageTranslations),
            })),
        ],
    };
}
...
async function getTranslations() {
    ...
    return await Promise.all(languageIds.map((languageId) => getNLS(extensionGalleryServiceUrl, resourceUrlTemplate, languageId, version).then((languageTranslations) => ({
        languageId,
...
async function main() {
    ...
    for (const { languageId, contents } of adml) {
        const languagePath = path.join(root, languageId === "en-us"
            ? "en-us"
