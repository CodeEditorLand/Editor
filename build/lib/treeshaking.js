...
function markNodes(ts, languageService, options) {
    ...
    function enqueue_black(node) {
        ...
        setColor(node, 2 /* NodeColor.Black */);
        black_queue.push(node);
        if (options.shakeLevel === 2 /* ShakeLevel.ClassMembers */ &&
    ...
    while (black_queue.length > 0 || gray_queue.length > 0) {
        ...
        if (black_queue.length === 0) {
            for (let i = 0; i < gray_queue.length; i++) {
                ...
                if ((ts.isClassDeclaration(nodeParent) ||
        ...
        const loop = (node) => {
            ...
            for (const { symbol, symbolImportNode } of symbols) {
                ...
                if (isSymbolWithDeclarations(symbol) &&
                    ...
                    for (let i = 0, len = symbol.declarations.length; i < len; i++) {
                        ...
                        if (options.shakeLevel === 2 /* ShakeLevel.ClassMembers */ &&
                            ...
                            !isLocalCodeExtendingOrInheritingFromDefaultLibSymbol(ts, program, checker, declaration)) {
    ...
    while (export_import_queue.length > 0) {
        ...
        if (aliased.declarations && aliased.declarations.length > 0) {
            if (nodeOrParentIsBlack(aliased.declarations[0]) ||
...
function generateResult(ts, languageService, shakeLevel) {
    ...
    program.getSourceFiles().forEach((sourceFile) => {
        ...
        function writeMarkedNodes(node) {
            ...
            if (ts.isImportDeclaration(node)) {
                if (node.importClause && node.importClause.namedBindings) {
                    if (ts.isNamespaceImport(node.importClause.namedBindings)) {
                        if (getColor(node.importClause.namedBindings) ===
                            2 /* NodeColor.Black */) {
                            return keep(node);
                        }
                    }
                        ...
                        for (const importNode of node.importClause.namedBindings
                            .elements) {
            ...
            if (shakeLevel === 2 /* ShakeLevel.ClassMembers */ &&
                ...
                for (let i = node.members.length - 1; i >= 0; i--) {
                    ...
                    toWrite =
                        toWrite.substring(0, pos) + toWrite.substring(end);
                }
...
function getRealNodeSymbol(ts, checker, node) {
    const getPropertySymbolsFromContextualType = ts
        .getPropertySymbolsFromContextualType;
    const getContainingObjectLiteralElement = ts
    ...
    if (symbol) {
        ...
        if (element) {
            ...
            if (contextualType) {
                const propertySymbols = getPropertySymbolsFromContextualType(element, checker, contextualType, 
                /*unionSymbolOk*/ false);
                if (propertySymbols) {
