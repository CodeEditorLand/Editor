		...
		try {
			...
			if (
				...
				if (
					...
					console.log(
						`Fetched response body buffer: ${ansiColors.magenta(`${assetContents.byteLength} bytes`)}`,
					);
					if (options.checksumSha256) {
						...
						if (actualSHA256Checksum !== options.checksumSha256) {
							throw new Error(
								`Checksum mismatch for ${ansiColors.cyan(asset.url)} (expected ${options.checksumSha256}, actual ${actualSHA256Checksum}))`,
							);
						}
					}
					console.log(
						`Verified SHA256 checksums match for ${ansiColors.cyan(asset.url)}`,
					);
				...
				throw new Error(
					`Request ${ansiColors.magenta(asset.url)} failed with status code: ${assetResponse.status}`,
				);
			}
			throw new Error(
				`Request ${ansiColors.magenta("https://api.github.com")} failed with status code: ${response.status}`,
			);
		} finally {
...
async function getVSCodeSysroot(arch) {
	...
	const sysroot =
		...
		path.join((0, os_1.tmpdir)(), `vscode-${arch}-sysroot`);
