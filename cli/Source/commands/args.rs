...
pub struct ServeWebArgs {
	...
	/// Run without a connection token. Only use this if the connection is
	/// secured by other means.
	#[clap(long)]
	pub without_connection_token:bool,
	/// If set, the user accepts the server license terms and the server will be
	/// started without a user prompt.
	#[clap(long)]
	pub accept_server_license_terms:bool,
	/// Specifies the path under which the web UI and the code server is
	/// provided.
	#[clap(long)]
	...
	/// Specifies the directory that user data is kept in. Can be used to open
...
pub struct CommandShellArgs {
	...
	/// Optional parent process id. If provided, the server will be stopped when
	/// the process of the given pid no longer exists
	#[clap(long, hide = true)]
...
pub struct ListExtensionArgs {
	/// Filters installed extensions by provided category, when using
	/// --list-extensions.
...
pub struct DesktopCodeOptions {
	...
	/// "insiders", a version number, or an absolute path to an existing
	/// install.
...
pub struct GlobalOptions {
	...
	/// accepted as part of the license prompt or specified in
	/// '--telemetry-level'
	#[clap(long, global = true, hide = true)]
...
pub struct EditorTroubleshooting {
	...
	/// Allow debugging and profiling of extensions. Check the developer tools
...
pub struct ExistingTunnelArgs {
	/// Name you'd like to assign preexisting tunnel to use to connect the
	/// tunnel Old option, new code should just use `--name`.
	#[clap(long, hide = true)]
...
pub struct BaseServerArgs {
	/// Requests that extensions be preloaded and installed on connecting
	/// servers.
	#[clap(long)]
...
pub struct TunnelForwardArgs {
	...
	/// Login args -- used for convenience so the forwarding call is a single
	/// action.
	#[clap(flatten)]
	pub login:LoginArgs,
}
