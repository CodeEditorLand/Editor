...
impl From<Vec<u8>> for SocketSignal {
	fn from(v:Vec<u8>) -> Self { SocketSignal::Send(v) }
}
...
impl Drop for ServerMessageSink {
	fn drop(&mut self) { self.multiplexer.remove(self.id); }
}
...
impl ClientMessageDecoder {
	pub fn new_plain() -> Self { ClientMessageDecoder { dec:None } }
...
impl FlateAlgorithm for DecompressFlateAlgorithm {
	fn total_in(&self) -> u64 { self.0.total_in() }

	fn total_out(&self) -> u64 { self.0.total_out() }
...
mod tests {
	...
	// Test that fixes #191501. Ensures compressed data can be streamed out
	// correctly.
	#[test]
