...
where
	T: std::fmt::Debug,
	S: Into<String>, {
	WrappedError { message:message.into(), original:format!("{:?}", original) }
}
...
// When the provided connection token doesn't match the one used to set up the
// original VS Code Server This is most likely due to a new user joining.
#[derive(Debug)]
...
// `makeAnyError!(FooError)`). Useful to easily deal with application error
// types without making tons of "From" clauses.
...
pub enum CodeError {
	...
	CommandFailed { command:String, code:i32, output:String },
