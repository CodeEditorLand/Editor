...
import { arrayEquals, PromiseAdapter, promiseFromEvent } from "./common/utils";
	...
	implements vscode.AuthenticationProvider, vscode.Disposable
{
	...
	constructor(
		...
		this._sessionsPromise = this.readSessions().then((sessions) => {
			...
			setTimeout(
				() => sessions.forEach((s) => this.afterSessionLoad(s)),
				1000,
			);
			return sessions;
		});
		...
		this._disposable = vscode.Disposable.from(
			...
			vscode.authentication.registerAuthenticationProvider(
				...
				this,
				{ supportsMultipleAccounts: this._supportsMultipleAccounts },
			),
	...
	async getSessions(
		...
		const accountFilteredSessions = options?.account
			? sessions.filter(
					(session) =>
						session.account.label === options.account?.label,
				)
			: sessions;
	...
	private async checkForUpdates() {
		...
		storedSessions.forEach((session) => {
			const matchesExisting = previousSessions.some(
				(s) => s.id === session.id,
			);
		...
		previousSessions.forEach((session) => {
			const matchesExisting = storedSessions.some(
				(s) => s.id === session.id,
			);
	...
	public async createSession(
		...
		try {
			...
			*/
			...
			this._logger.info(
				`Logging in with '${loginWith ? loginWith : "any"}' account...`,
			);
	...
	private _shouldSupportMultipleAccounts(): boolean {
		...
		const inspect = vscode.workspace
			.getConfiguration("github.experimental")
			.inspect<boolean>("multipleAccounts");
		if (inspect?.workspaceFolderValue !== undefined) {
			this._logger.trace(
				`Acquired multi-account enablement value from 'workspaceFolderValue'. Value: ${inspect.workspaceFolderValue}`,
			);
			return inspect.workspaceFolderValue;
		}
		if (inspect?.workspaceValue !== undefined) {
			this._logger.trace(
				`Acquired multi-account enablement value from 'workspaceValue'. Value: ${inspect.workspaceValue}`,
			);
			return inspect.workspaceValue;
		}
		if (inspect?.globalValue !== undefined) {
			this._logger.trace(
				`Acquired multi-account enablement value from 'globalValue'. Value: ${inspect.globalValue}`,
			);
		...
		this._logger.trace(
			`Acquired multi-account enablement value from default. Value: ${value} because of uriScheme: ${vscode.env.uriScheme}`,
		);
