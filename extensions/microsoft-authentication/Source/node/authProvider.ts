...
export class MsalAuthProvider implements AuthenticationProvider {
	...
	async initialize(): Promise<void> {
		await this._eventBufferer.bufferEventsAsync(() =>
			this._publicClientManager.initialize(),
		);
	...
	async getSessions(
		...
		const cachedPca = await this.getOrCreatePublicClientApplication(
			scopeData.clientId,
			scopeData.tenant,
		);
			...
			if (e instanceof CancellationError) {
				...
				const result = await window.showErrorMessage(
					l10n.t("Having trouble logging in?"),
					{
						modal: true,
						detail: l10n.t(
							"Would you like to try a different way to sign in to your Microsoft account? ({0})",
							"protocol handler",
						),
					},
					yes,
	...
	private async getOrCreatePublicClientApplication(
		clientId: string,
		tenant: string,
	...
	private sessionFromAccountInfo(
		account: AccountInfo,
	): AuthenticationSession {
		return {
			accessToken: "1234",
