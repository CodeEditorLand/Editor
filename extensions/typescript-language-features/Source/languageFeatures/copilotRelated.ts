							...
							relatedAPI.registerRelatedFilesProvider(
								...
								async (uri, _context, token) => {
									...
									try {
										...
										if (!vscode.window.activeTextEditor) {
											vscode.window.showErrorMessage(
												vscode.l10n.t(
													"Related files provider failed. No active text editor.",
												),
											);
											return { entries: [] };
										}
									...
									if (!isSupportedLanguageMode(document)) {
										vscode.window.showErrorMessage(
											vscode.l10n.t(
												"Related files provider failed. Copilot requested file with unsupported language mode.",
											),
										);
										return { entries: [] };
									}
									...
									const file =
										client.toOpenTsFilePath(document);
									if (!file) {
										return { entries: [] };
									}
									...
									const response = (await client.execute(
										...
										{ file },
										token,
									)) as Proto.CopilotRelatedResponse;
									if (
