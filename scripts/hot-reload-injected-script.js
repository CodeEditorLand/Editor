...
function setupGlobals(vscode) {
	...
	const store =
		globalThis["hot-reload-injected-script-disposables"] ??
		(globalThis["hot-reload-injected-script-disposables"] =
			new DisposableStore());
	store.clear();
	...
	function getConfig() {
		const config = vscode.workspace
			.getConfiguration("vscode-diagnostic-tools")
			.get("debuggerScriptsConfig", {
				"hotReload.sources": {},
			});
	...
	const api = {
		...
		getConfig: (relativePath) => {
			...
			return {
				mode:
					config["hotReload.sources"][relativePath] ===
					"patch-prototype"
						? "patch-prototype"
						: undefined,
			};
	...
	function update() {
		...
		const part = e.document.fileName
			.replace(/\\/g, "/")
			.replace(/\.ts/, ".js")
			.split("/src/")[1];
		if (!part) {
	...
	store.add(
		vscode.commands.registerCommand(
			"vscode-diagnostic-tools.hotReload.toggle",
			async (relativePath) => {
				let config = getConfig();
				const current = config["hotReload.sources"][relativePath];
				const newValue =
					current === "patch-prototype"
						? undefined
						: "patch-prototype";
				config = {
					...config,
...
module.exports.run = async function (debugSession, ctx) {
	...
	const watcher = store.add(
		await DirWatcher.watchRecursively(path.join(__dirname, "../out/")),
	);
	...
	store.add(
		watcher.onDidChange(async (changes) => {
			const supportedChanges = changes
				.filter(
					(c) => c.path.endsWith(".js") || c.path.endsWith(".css"),
				)
				.map((c) => {
					const relativePath = c.path
						.replace(/\\/g, "/")
						.split("/out/")[1];
					return {
						...c,
						relativePath,
						config: global?.getConfig(relativePath),
					};
				});

			const result = await debugSession.evalJs(
				function (changes, debugSessionName) {
					...
					function handleJsChange(
						...
						const g = /** @type {GlobalThisAddition} */ (
							globalThis
						);

						// A frozen copy of the previous exports
						const oldExports = Object.freeze({
							...oldModule.exports,
						});
						...
						newScript(
							/* define */ function (deps, callback) {
								...
								const newModule =
									moduleManager._modules2[moduleId];
			...
			for (const failedFile of result.reloadFailedJsFiles) {
				const reloadFn =
					reloadFns.get(failedFile.relativePath) ??
					(() => {
		...
		return new DirWatcher(
			...
			(path) => {
				...
				if (content !== undefined) {
					listeners.forEach((l) =>
						l([{ path: path, newContent: content }]),
					);
				}
			},
		);
